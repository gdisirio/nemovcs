...
class StatusDialog(Gtk.Window):
    def __init__(self, paths: Sequence[str]):
        ...
        files_header.pack_start(refresh, False, False, 0)
        ...
        summary_scroll.set_shadow_type(Gtk.ShadowType.IN)
        summary_scroll.add(self.tree)
        files_box.pack_start(summary_scroll, True, True, 0)
        notebook.append_page(files_box, Gtk.Label(label="Summary"))
        ...
        output_scroll.set_shadow_type(Gtk.ShadowType.IN)
        output_scroll.add(self.output_view)
        notebook.append_page(output_scroll, Gtk.Label(label="Output"))
